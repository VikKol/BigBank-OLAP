...
def post_task(args, task_json, timeout_at):
  try:
    ...
    if isinstance(e, urllib2.HTTPError) and e.code >= 400 and e.code <= 500:
      ...
      sys.stderr.write("Redirect response received, retrying with url [{0}]\n".format(args.url))
      time.sleep(5)
...
def await_task_completion(args, task_id, timeout_at):
  while True:
    url = args.url.rstrip("/") + "/druid/indexer/v1/task/{0}/status".format(urllib2.quote(task_id))
